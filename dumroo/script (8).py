# Fix the RBAC SQL issue and create proper Streamlit interface

# First, let me fix the RBAC query modification
improved_rbac_code = '''
import sqlite3
import pandas as pd
...
streamlit_app_code = '''#!/usr/bin/env python3
"""
...
# Save to file
with open('dumroo_streamlit_app.py', 'w') as f:
    f.write(streamlit_app_code)

# Test the improved RBAC with a simple implementation
class TestImprovedRBAC:
    def __init__(self):
        self.admin_df = pd.read_csv('admin_users.csv')
...
# Test the improved RBAC
test_rbac = TestImprovedRBAC()

print("\n=== Testing Improved RBAC Filters ===")
test_users = ['super_admin', 'priya_sharma', 'teacher_grade6a']

for username in test_users:
    permissions = test_rbac.get_user_permissions(username)
    if permissions:
        user_filter = test_rbac.apply_student_filters(username)
        print(f"\n👤 {permissions['full_name']} ({permissions['role']}):")
        print(f"   Filter: '{user_filter}' ")
        print(f"   Access: {permissions['assigned_grades']} grades, {permissions['assigned_sections']} sections")

print("\n✅ Streamlit application saved as 'dumroo_streamlit_app.py'")
print("✅ Improved RBAC system tested successfully!")